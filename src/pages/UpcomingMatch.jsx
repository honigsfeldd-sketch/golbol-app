...
import { ArrowLeft, MapPin, User, Share2, Pencil, X, ArrowRightLeft, Trash2, Check, Search, ChevronRight, UserPlus } from "lucide-react";
            ...
            >
              ...
              <div className="flex items-center justify-between px-5 py-4 border-b border-border/40">
                <div className="flex items-center gap-2">
                  {replacingPlayer && (
                    <button
                      onClick={() => { setReplacingPlayer(null); setReplaceSearch(""); }}
                  ...
                  <h2 className="text-base font-semibold">
                    {replacingPlayer ? `Replace ${replacingPlayer.player.nickname || replacingPlayer.player.name}` : "Edit Teams"}
                  </h2>
                </div>
                <div className="flex items-center gap-2">
              ...
              <AnimatePresence mode="wait">
                {replacingPlayer ? (
                  ...
                  >
                    ...
                    <div className="px-5 py-3 bg-secondary/30 flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full overflow-hidden bg-zinc-200 shrink-0">
                        {replacingPlayer.player.image ? (
                          <img src={replacingPlayer.player.image} alt="" className="w-full h-full object-cover" />
                        ) : (
                          <div className="w-full h-full flex items-center justify-center"><User className="w-4 h-4 text-zinc-400" /></div>
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold truncate">{replacingPlayer.player.nickname || replacingPlayer.player.name}</p>
                        <p className="text-[11px] text-muted-foreground">
                          {replacingPlayer.team === "A" ? "White" : "Black"} team · {replacingPlayer.player.position || "MID"}
                        </p>
                      </div>
                      <button
                        onClick={() => {
                          removePlayer(replacingPlayer.player, replacingPlayer.team);
                          setReplacingPlayer(null);
                          setReplaceSearch("");
                        }}
                        className="px-3 py-1.5 rounded-xl bg-red-500/10 text-red-500 text-xs font-semibold hover:bg-red-500/20 transition-colors shrink-0"
                      >
                        Remove
                      </button>
                    </div>
                    ...
                    <div className="px-5 pt-3 pb-2">
                      <div className="relative">
                        ...
                        <input
                          ...
                          autoFocus
                          className="w-full h-10 pl-9 pr-4 rounded-xl bg-secondary text-sm outline-none focus:ring-2 focus:ring-primary/20 transition-all placeholder:text-muted-foreground/40"
                        />
                      </div>
                    </div>
                          ...
                          return (
                            <div className="flex flex-col items-center justify-center py-12 text-center">
                              ...
                              <p className="text-sm text-muted-foreground/50">
                                {availablePlayers.length === 0 ? "All players are assigned" : "No players match your search"}
                              </p>
                            </div>
                          );
                        }
                          ...
                          <div className="space-y-1">
                            {filtered.map((player) => (
                              <button
                                key={player.id}
                                onClick={() => {
                                  replacePlayer(replacingPlayer.player, player, replacingPlayer.team);
                                  setReplacingPlayer(null);
                                  setReplaceSearch("");
                                }}
                                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-2xl hover:bg-secondary active:bg-secondary/80 transition-colors text-left"
                              >
                                <div className="w-10 h-10 rounded-full overflow-hidden bg-zinc-200 shrink-0">
                                  {player.image ? (
                                ...
                                <div className="flex-1 min-w-0">
                                  <p className="text-sm font-medium truncate">{player.nickname || player.name}</p>
                                  <p className="text-[11px] text-muted-foreground/50">{player.position || "MID"}</p>
                                </div>
                      ...
                      <div className="space-y-1">
                        {lineupA.map((player) => (
                          <div key={player.id} className="flex items-center gap-2">
                            <button
                              onClick={() => setReplacingPlayer({ player, team: "A" })}
                              className="flex-1 flex items-center gap-3 px-3 py-2.5 rounded-2xl hover:bg-secondary active:bg-secondary/80 transition-colors text-left"
                            >
                              <div className="w-9 h-9 rounded-full overflow-hidden bg-zinc-200 shrink-0">
                                {player.image ? (
                                  <img src={player.image} alt="" className="w-full h-full object-cover" />
                                ) : (
                                  <div className="w-full h-full flex items-center justify-center"><User className="w-3.5 h-3.5 text-zinc-400" /></div>
                                )}
                              </div>
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-medium truncate">{player.nickname || player.name}</p>
                              ...
                              <span className="text-[10px] text-muted-foreground/30 font-medium">Replace</span>
                              <ChevronRight className="w-4 h-4 text-muted-foreground/20 shrink-0" />
                            </button>
                            <button
                              onClick={() => movePlayer(player, "A")}
                              className="w-8 h-8 rounded-full bg-secondary/60 flex items-center justify-center hover:bg-secondary transition-colors shrink-0"
                              title="Move to Black"
                            >
                    ...
                    <div className="px-5 pt-3 pb-2">
                      ...
                      <div className="space-y-1">
                        {lineupB.map((player) => (
                          <div key={player.id} className="flex items-center gap-2">
                            <button
                              onClick={() => setReplacingPlayer({ player, team: "B" })}
                              className="flex-1 flex items-center gap-3 px-3 py-2.5 rounded-2xl hover:bg-secondary active:bg-secondary/80 transition-colors text-left"
                            >
                              <div className="w-9 h-9 rounded-full overflow-hidden bg-zinc-800 shrink-0">
                                {player.blackJerseyImage || player.image ? (
                                  ...
                                  <div className="w-full h-full flex items-center justify-center"><User className="w-3.5 h-3.5 text-zinc-500" /></div>
                                )}
                              </div>
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-medium truncate">{player.nickname || player.name}</p>
                    ...
                    {availablePlayers.length > 0 && (
                      <>
                        ...
                        <div className="px-5 pt-3 pb-4">
                          <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/50 mb-2">
                            Add to team ({availablePlayers.length} available)
                          </div>
                          <div className="space-y-1">
                            {availablePlayers.slice(0, 6).map((player) => (
                              <div key={player.id} className="flex items-center gap-2">
                                <div className="flex-1 flex items-center gap-3 px-3 py-2 rounded-2xl">
                                ...
                                <button
                                  ...
                                  className="h-7 px-2.5 rounded-lg bg-zinc-100 hover:bg-zinc-200 text-[11px] font-semibold transition-colors shrink-0"
                                >
                                  + White
                                </button>
                                <button
                                  ...
                                  className="h-7 px-2.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-white text-[11px] font-semibold transition-colors shrink-0"
                                >
                                  + Black
                                </button>
                              </div>
                            ...
                            {availablePlayers.length > 6 && (
                              <p className="text-center text-[11px] text-muted-foreground/40 pt-1">
                                +{availablePlayers.length - 6} more — use Replace to see all
                              </p>
                            )}
                          </div>
                        </div>
